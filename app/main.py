...
def create_event(payload: EventCreate, db: Session = Depends(get_db)):
    event=Event(**payload.model_dump()); db.add(event); db.commit(); db.refresh(event); return event
...
def summary(db: Session=Depends(get_db)):
    total,users,total_value,avg_value=db.execute(select(func.count(Event.id),func.count(func.distinct(Event.user_id)),func.coalesce(func.sum(Event.value),0.0),func.coalesce(func.avg(Event.value),0.0))).one()
